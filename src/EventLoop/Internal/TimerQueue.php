...
final class Timer_Queue
{
     ...
     */
    public function insert(Timer_Callback $callback): void
    {
     ...
     * Time complexity: O(log(n)).
     */
    public function remove(Timer_Callback $callback): void
    {
        $id = $callback->id;
        if (!isset($this->pointers[$id])) {
            return;
        }
        $this->remove_and_rebuild($this->pointers[$id]);
    }
    /**
     ...
     */
    public function extract(float $now): ?Timer_Callback
    {
        if (!$this->callbacks) {
            return null;
        }
     ...
     */
    private function heapify_up(int $node): void
    {
     ...
     */
    private function heapify_down(int $node): void
    {
     ...
     */
    private function remove_and_rebuild(int $node): void
    {
